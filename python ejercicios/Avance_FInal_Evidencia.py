...
Datos.drop(["maxima_base_interrumpible", "minima_base_interrumpible", "volumetrica", "fecha_inicio","fecha_fin"],axis=1,inplace=True)
...
miCursor.execute('CREATE DATABASE Bases')
miCursor.execute('USE Bases')
miCursor.execute("CREATE TABLE RegistroBase (zona VARCHAR(30), capacidad_base_firme FLOAT, uso_base_firme FLOAT, capacidad_base_temporal FLOAT, uso_base_temporal FLOAT)")
...
for x, row in Datos.iterrows():
    sql = 'INSERT INTO RegistroBase (zona, capacidad_base_firme, uso_base_firme, capacidad_base_temporal, uso_base_temporal) VALUES (%s, %s, %s, %s, %s)'
